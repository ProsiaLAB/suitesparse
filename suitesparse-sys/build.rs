...
impl Library {
    ...
    fn find_include_dir() -> Option<String> {
        ...
        for path in &[
            ...
            if PathBuf::from(format!("{path}/klu.h")).exists() {
    ...
    fn find_lib_dir() -> Option<String> {
        ...
        for path in &["/usr/lib", "/usr/local/lib", "/usr/lib/x86_64-linux-gnu"] {
            if PathBuf::from(format!("{path}/libsuitesparseconfig.{check_ext}")).exists() {
                return Some(path.to_string());
            }
        }
...
fn build_vendor() -> Library {
    ...
    if static_libraries == "OFF" && shared_libraries == "OFF" {
        println!(
            "cargo:warning=Both static_libraries and shared_libraries features are disabled. Defaulting to static_libraries."
        );
        static_libraries = "ON";
    }
    ...
    let mut config = cmake::Config::new("vendor/suitesparse");
    config
    ...
    let inc_dir = Some(format!("{dst_disp}/include/suitesparse"));
    Library {
        inc: inc_dir,
        lib: lib_loc,
...
fn generate_bindings(suitesparse: &Library) -> Result<(), String> {
    ...
    // Add search paths for dependencies like GMP
    let search_paths = if cfg!(target_os = "macos") {
        vec!["/usr/local/include", "/opt/homebrew/include"]
    } else {
        vec!["/usr/include", "/usr/local/include"]
    };
...
fn main() -> Result<(), String> {
    ...
    for &lib in ENABLED_LIBRARIES {
        if lib == "mongoose" {
            lib_names.push("suitesparse_mongoose");
        } else {
            lib_names.push(lib);
        }
    }
